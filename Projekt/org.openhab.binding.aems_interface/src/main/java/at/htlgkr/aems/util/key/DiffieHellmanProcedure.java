...
public class DiffieHellmanProcedure {
     ...
     * @param secret - the secret number which is customarily between {@code SECRET_BOTTOM_LIMIT} and
     *            {@code SECRET_TOP_LIMIT} (each inclusively)
    ...
    public static byte[] receiveKeyInfos() throws IOException {
        ...
        try {
            ...
            // don't attempt to connect to the client side since port forwarding can not be configured at the client
            // side!!!
            try {
                BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));
    ...
    public static byte[] confirmKey() throws IOException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()))) {
            String input = reader.readLine();
            Matcher matcher = Pattern.compile(".*combination:(?<combination>\\d+)").matcher(input);
            matcher.find();
            BigDecimal combination = new BigDecimal(matcher.group("combination"));
    ...
    public static String prepareKeyAcquisition(String ip) throws Exception {
        HttpURLConnection con = (HttpURLConnection) new URL("http://" + ip + ":8084/AEMSWebService/AAA")
                .openConnection();
